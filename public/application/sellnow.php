...
try {
	...
	//procesor name
	if ($payment_method == '0' || $payment_method == '5')
		$processorName = "PP";
				...
				$data = array(
				  ...
				  $insertsql -> execute($data);
					...
					else
					{
						...
						// Send admin email with details.
						$getRadiusLogs = $rad -> prepare("select `acctstarttime`, `acctstoptime`, `acctsessiontime`, `callingstationid` from `radacct` where `username` = :id");
						$getRadiusLogs -> execute(array(':id' => $user_id));

						$Logs = array_map(function ($row) {
						    return sprintf('%s %s %s %s', $row['acctstarttime'], $row['acctstoptime'], $row['acctsessiontime'], $row['callingstationid']);
						}, $getRadiusLogs -> fetchAll(PDO::FETCH_ASSOC));

						$LogsString = implode("<br>", $Logs); 

						if (empty($LogsString)) {
							    $LogsString = 'There are no logs to report';
						}

						$mailcontent = 'A user has recently disputed a payment. The details can be found below
										<hr>
										Account: <a href="https://crypticvpn.com/ydir/users.php?id='.$user_id.'">'.$userName.'('.$user_id.')</a><br>
										Email: '.$buyer_email.'<br>
										Purchase IP: '.$buyer_ip.'<br>
										Payment Method: '.$processorName.'<br>
										Transaction ID: '.$txn_id.'<br>
										Amount: '.$invoice_amount_usd.'<br>
										Order #: <a href="https://rocketr.net/seller/orders/'.$order_id.'">'.$order_id.'</a><br>
										Package: '.$product_title.'<br>
										<hr>
										The buyer made the purchase from the IP address '.$buyer_ip.' which geo-locates to the address the buyer has listed on their PayPal account. They have used our product multiple times and it seems that this dispute is just an attempt to abuse the PayPal system and keep our product while trying to get their money back.
										<hr>
										Usage Logs can be found below<br>
										'.$LogsString.'';
						...
						$mail->Subject = 'Dispute Details for '.$userName.'('.$user_id.')';
						$mail->msgHTML($mailcontent);
						$mail->send();
						errorAndDie("Email sent\n");
