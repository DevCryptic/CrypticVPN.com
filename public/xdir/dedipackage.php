...
<head>
    ...
    <title><?php echo $site_title; ?> - Dedicated Packages</title>
			...
			<?php
				if (isset($_POST['addPackage']))
				{
					...
					$errors = array();
						...
						if(empty($errors)) {
							$insertPackage = $odb -> prepare("INSERT INTO `dpackages` VALUES (NULL, :name, :details, :length, :unit, :spots, :price)");
							$insertPackage -> execute(array(':name' => $name, ':price' => $price, ':unit' => $unit, ':length' => $length, ':details' => $details, ':spots' => $spots));
                ...
                <div class="md-card-content">
                    <div class="uk-overflow-container">
                        <table class="uk-table">
                            ...
                            <tbody>
                           <?php
								$GetPackages = $odb -> prepare("SELECT * FROM `dpackages` ORDER BY `id` DESC LIMIT 100");
								$GetPackages -> execute();
								while ($getInfo = $GetPackages -> fetch(PDO::FETCH_ASSOC))
								{
									...
									echo '
									<tr>
										<td>'.$name.' ('.$spots.' Spots)</td>
										<td>'.$length.' '.$unit.' </td>
										<td>'.$price.'</td>
										<td><a class="md-btn md-btn-primary" href="viewdpackage.php?id='.$id.'">View</a></td>
									</tr>';
								}
							?>
                            </tbody>
