...
$checkIfExists = $odb -> prepare("SELECT * FROM `dpackages` WHERE `id` = :packageid");
$checkIfExists -> execute(array(':packageid' => $packageid));
if($checkIfExists -> rowCount() == 0)
{
	header('location: dedicatedsales.php');
	die();
}

$SQLGetInfo = $odb -> prepare("SELECT * FROM `dpackages` WHERE `id` = :id LIMIT 1");
        ...
        <div id="page_content_inner">
			...
			<?php
				if (isset($_POST['updatePackage'])) {
					...
					if ($name != $_POST['name'])
					{
						$SQL = $odb -> prepare("UPDATE `dpackages` SET `name` = :name WHERE `id` = :id");
					...
					{
						$SQL = $odb -> prepare("UPDATE `dpackages` SET `price` = :price WHERE `id` = :id");
						$SQL -> execute(array(':price' => $_POST['price'], ':id' => $packageid));
						$update = true;
						$price = $_POST['price'];
					}
					if ($details != $_POST['details'])
					{
						$SQL = $odb -> prepare("UPDATE `dpackages` SET `details` = :details WHERE `id` = :id");
						$SQL -> execute(array(':details' => $_POST['details'], ':id' => $packageid));
						$update = true;
						$details = $_POST['details'];
					}
					if ($length != $_POST['length'])
					{
						$SQL = $odb -> prepare("UPDATE `dpackages` SET `length` = :length WHERE `id` = :id");
					...
					{
						$SQL = $odb -> prepare("UPDATE `dpackages` SET `spots` = :spots WHERE `id` = :id");
						$SQL -> execute(array(':spots' => $_POST['spots'], ':id' => $packageid));
						$update = true;
						$spots = $_POST['spots'];
					}
					if ($unit != $_POST['unit'])
					{
						$SQL = $odb -> prepare("UPDATE `dpackages` SET `unit` = :unit WHERE `id` = :id");
				...
				if (isset($_POST['deletePackage'])) {
					$sql = $odb -> prepare("DELETE FROM `dpackages` WHERE `id` = :id");
			...
			<form method="post">
                <div class="md-card-content">
                    <h3 class="heading_a">View & Manage Dedicated Package - <?php echo $name; ?></h3>
                    <div class="uk-grid" data-uk-grid-margin>
                        ...
                        <div class="uk-width-medium-1-2">
                            <div class="uk-form-row">
                                <label>Details</label>
                                <textarea cols="30" rows="4" class="md-input label-fixed" name="details"><?php echo $details; ?></textarea>
                            </div>
                        </div>
                    </div>
                    <div class="uk-grid">
